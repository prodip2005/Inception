...
const Members = () => {
    ...
    useEffect(() => {
        Promise.all([
            fetchData('/institutions.json', setInstCount),
            fetchData('/peoples.json', setPeopleCount)
                ...
                <header className="mb-12 md:mb-20 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/10 pb-8 md:pb-12">
                    <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }}>
                        <div className="flex items-center gap-3 mb-4 md:mb-6">
                            <div className="h-[1px] md:h-[2px] w-8 md:w-12 bg-[#8a0001]"></div>
                            <span className="text-[#8a0001] font-mono text-[10px] md:text-xs tracking-[0.3em] uppercase animate-pulse">Data_Sync_Active</span>
                        </div>
                        <h2 className="text-5xl md:text-8xl font-black bg-clip-text text-transparent bg-gradient-to-r from-[#8a0001] via-red-500 to-white leading-none tracking-tighter uppercase">
                            NETWORK<span className="text-[#8a0001] animate-pulse">_</span>
                        </h2>
                    </motion.div>
                    <div className="text-right font-mono hidden md:block text-slate-500 text-[10px]">
                        ...
                        <div className="flex items-center gap-2 text-white/80 text-sm">
                            <FaDatabase className="text-[#8a0001]" />
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-10">
                    {categories.map((item, idx) => (
                        <motion.div
                            ...
                            className="group relative p-0.5 md:p-1 border border-white/5 rounded-[1.5rem] md:rounded-[3rem] bg-white/[0.03] backdrop-blur-2xl transition-all duration-500 hover:border-[#8a0001]/30 h-full"
                        >
                            <div className="p-4 md:p-10 flex flex-col h-full relative overflow-hidden">
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-16 gap-2">
                                    <div className="p-3 md:p-5 bg-white/5 rounded-xl md:rounded-2xl border border-white/10 text-white/80 group-hover:text-[#8a0001] transition-all">
                                    ...
                                    <div className="text-left md:text-right">
                                        ...
                                        <div className="font-mono text-[8px] md:text-[9px] text-slate-500 tracking-widest uppercase mt-1">{item.label}</div>
                                    </div>
                                </div>

                                <h3 className="text-lg md:text-4xl font-black text-white mb-2 md:mb-4 tracking-tighter uppercase">{item.title}</h3>
                                ...
                                {/* Bottom Action Button - Now Dark Crimson by Default */}
                                <div className="mt-auto pt-4 md:pt-8 border-t border-white/10">
                                    <button
                                        onClick={() => navigate(item.path)}
                                        className="group/btn relative flex items-center justify-between w-full md:w-auto md:min-w-[190px] px-4 md:px-6 py-2.5 md:py-3.5 bg-[#8a0001] border border-[#8a0001] rounded-full text-white overflow-hidden transition-all duration-300 hover:bg-[#a00001] hover:shadow-[0_0_25px_rgba(138,0,1,0.5)] active:scale-95"
                                    >
                ...
                <div className="mt-12 md:mt-20 flex justify-center">
                    <div className="px-4 py-1.5 md:px-6 md:py-2 bg-white/5 rounded-full border border-white/5 flex items-center gap-3 text-slate-500 font-mono text-[8px] md:text-[9px] uppercase tracking-widest">
                        <div className="flex gap-1">
                            {[1, 2, 3].map(i => <div key={i} className="w-1 h-1 bg-[#8a0001] rounded-full animate-bounce" style={{ animationDelay: `${i * 0.2}s` }}></div>)}
                        </div>
                        Nodes_Secure
                    </div>
                </div>
